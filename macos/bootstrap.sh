...
if [ ! -d "$HOME/docs" ]; then
    git clone ssh://git@omphalos.io:2222/mr-x/docs.git "$HOME/docs" \
        || echo "Warning: could not clone mr-x/docs (forge unreachable?) - skipping agent wiring"
fi
# Point every installed CLI agent at the shared instructions in ~/docs/agents/.
# Per-machine and not synced, so this runs on each box.
if [ -f "$HOME/docs/agents/core.md" ]; then
    "$DOTDIR/scripts/wire-agent-instructions.sh"
fi
