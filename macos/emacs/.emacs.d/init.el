...
yabai placement can join on it instead of on titles or creation order."
  ...
  (defun mr-x/save-session-state ()
    ...
    (interactive)
    (let ((frame-data (mr-x/--session-frames)))
...
Skips when there are no eligible frames so a bare daemon doesn't
clobber the last useful snapshot.  When this daemon was launched through
the diagnostic helper, the same frames also commit a coherent capture in
the run directory; placement stays frame-only there until exact-ID yabai
capture lands, so the legacy yabai mirror below is still the only
placement record."
    (when (mr-x/--session-eligible-frames)
      (let ((inhibit-message t)
            (message-log-max nil))
        (mr-x/save-session-state)
        (when (fboundp 'mr-x/crash-runtime-capture)
          (mr-x/crash-runtime-capture #'mr-x/--session-frames nil
                                      #'mr-x/--session-frame-keys)))
