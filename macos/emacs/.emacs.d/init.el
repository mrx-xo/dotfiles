...
(with-eval-after-load 'org-roam
 ...
 (defun my/org-roam-renew-copied-ids ()
   "Give existing IDs in the copied journal subtree fresh identities."
   (save-excursion
     (org-map-entries
      (lambda ()
 ...
 (defun my/org-roam-copy-todo-to-today ()
   ...
   (let ((org-refile-keep t) ;; Set this to nil to delete the original!
	    ...
	    (org-after-refile-insert-hook
           '(my/org-roam-renew-copied-ids save-buffer))
	    today-file
	    pos)
...
public dotfiles on purpose. nil when the file is missing or empty."
  ...
  ;; PERCENT-COMPLETE:0 and, on import, trusts the percent whenever it is
  ;; present, so a task written from org and ticked by HA reads back as 0
  ;; percent, TODO, forever. Let STATUS win. (Found 2026-09-09, Phase 2.)
  (defun mr-x/arca-caldav--completed-wins (eventdata)
    "Return EVENTDATA with percent-complete forced to \"100\" when status is COMPLETED."
    (if (and (equal (alist-get 'status eventdata) "COMPLETED")
             (not (equal (alist-get 'percent-complete eventdata) "100")))
        (cons (cons 'percent-complete "100")
...
still limits the actual styling to E Ink frames."
  ...
  ;; Every `org-mode' activation ends with (set-face-foreground 'org-hide BG),
  ;; where BG is whatever the *selected* frame's background is — a global
  ;; attribute write (frame nil), so it lands on every frame at once and
  ;; outranks face specs, override specs included. Open any org buffer with a
  ;; dark GUI frame selected and org-hide turns near-black everywhere; on
  ;; CALLIOPE's white e-ink the "hidden" leading stars come back as dark
  ;; asterisks. Re-derive the value per frame from that frame's own background
  ;; after each activation.
  (defun mr-x/org-hide-sync-to-frames (&rest _)
    "Repaint `org-hide' per frame to match that frame's own background."
    (dolist (frame (frame-list))
      (let ((bg (face-attribute 'default :background frame)))
        (when (and (stringp bg) (not (string-prefix-p "unspecified" bg)))
          (set-face-attribute 'org-hide frame :foreground bg)))))

  (advice-add 'org-mode :after #'mr-x/org-hide-sync-to-frames)
  (add-hook 'after-make-frame-functions #'mr-x/org-hide-sync-to-frames)
  (mr-x/org-hide-sync-to-frames)
...
(use-package exec-path-from-shell
  ...
  ;; nvm-installed CLIs (claude-agent-acp, acp-multiplex, MCP servers) live in
  ;; ~/.nvm/versions/node/<ver>/bin, which nvm exports only from an *interactive*
  ;; shell (.zshrc).  exec-path-from-shell uses a *login* shell, so a launchd
  ;; daemon never sees them and agent-shell errors "claude-agent-acp not found".
  ;; Unless a copy is already on PATH (e.g. a brew-global /opt/homebrew/bin one,
  ;; as on MrX), splice in the newest nvm node bin that actually holds the binary.
  (unless (executable-find "claude-agent-acp")
    (let ((dir (car (seq-filter
                     (lambda (d) (file-exists-p (expand-file-name "claude-agent-acp" d)))
                     (sort (file-expand-wildcards "~/.nvm/versions/node/*/bin")
...
(use-package no-littering
  ...
  (no-littering-theme-backups))
    ...
    (use-package multi-vterm
      ...
      ;; One-shot command runner: the middle ground between
      ;; `async-shell-command' (dumb output, no TTY) and a hand-driven
      ;; vterm popup (real terminal, but you type and clean up yourself).
      ;; Prompts for a command, runs it in a vterm popup that popper
      ;; already tracks (vterm-mode rule), and leaves focus where it was.
      ;; The shell stays alive after the command so the output persists
      ;; until dismissed; the next run kills the previous popup.
      (defvar mr-x/vterm-run-history nil
        "Minibuffer history for `mr-x/vterm-run'.")

      (defun mr-x/vterm-run (command)
        "Run COMMAND in a fresh vterm popup and keep focus in the current window.
With a prefix argument, re-run the previous command without prompting.
Any earlier run popup is killed first so they never pile up; dismiss the
popup with `popper-toggle' or kill it like any vterm buffer."
        (interactive
         (list (if (and current-prefix-arg (car mr-x/vterm-run-history))
                   (car mr-x/vterm-run-history)
                 (read-shell-command "Run: " nil 'mr-x/vterm-run-history))))
        (when (string-empty-p (string-trim command))
          (user-error "No command"))
        (dolist (buf (buffer-list))
          (when (string-prefix-p "*run: " (buffer-name buf))
            (when-let ((proc (get-buffer-process buf)))
              (set-process-query-on-exit-flag proc nil))
            (kill-buffer buf)))
        (let ((origin (selected-window))
              (buf (vterm-other-window
                    (format "*run: %s*" (truncate-string-to-width command 40 nil nil "...")))))
          (with-current-buffer buf
            (when-let ((proc (get-buffer-process buf)))
              (set-process-query-on-exit-flag proc nil))
...
current window layout."
  ...
  ;; `persp-reactivate-buffers' reorders the buffer list by calling
  ;; `switch-to-buffer' on every buffer in the perspective, trusting the
  ;; following `set-window-configuration' to undo the display.  With
  ;; `switch-to-buffer-obey-display-actions' those calls become real
  ;; display-buffer calls: popup buffers pop, and minibuffer buffers
  ;; (which perspective's set-window-buffer advice adds to the persp)
  ;; get split into whichever frame display-buffer picks -- on frame
  ;; delete that was the MAIN frame, which came back with a giant
  ;; " *Minibuf-1*" window and a *Messages* popup.  Restore the plain
  ;; set-window-buffer semantics perspective was written against.
  (define-advice persp-reactivate-buffers (:around (fn buffers) plain-switch)
...
inside display-buffer before major-pane may be loaded."
  ...
  ;; Killing or burying a buffer must never hand a main window a pane
  ;; conversation.  `switch-to-prev-buffer' (used by kill-buffer,
  ;; bury-buffer and quit-window to pick the replacement) walks the
  ;; window's history, and the conversation is usually in it, so the
  ;; same buffer ended up on screen twice.  Skip two kinds of buffer:
  ;; anything already visible on some frame, and -- for every window
  ;; but the pane itself -- any pane conversation, on screen or not.
  ;; A FUNCTION value matters here: the symbol values (`this',
  ;; `visible') fall back to the first skipped buffer when nothing else
  ;; is left, which is exactly the duplicate we are avoiding.
  (defun mr-x/prev-buffer-skip-p (window buffer _bury-or-kill)
    "Non-nil when BUFFER must not replace what WINDOW shows.
Rejects buffers visible on any frame and, unless WINDOW is the
major-pane, every pane conversation whether displayed or not."
    (or (get-buffer-window buffer 'visible)
        (and (not (window-parameter window 'major-pane))
             (if (fboundp 'major-pane--conversation-p)
                 (major-pane--conversation-p buffer)
               (mr-x/agent-pane-buffer-p (buffer-name buffer) nil)))))

  (setq switch-to-prev-buffer-skip #'mr-x/prev-buffer-skip-p)

  ;; The function form has a hole: when every candidate is skipped,
  ;; `switch-to-prev-buffer' still returns non-nil (its NEW-BUFFER
  ;; variable holds the last rejected candidate), so
  ;; `replace-buffer-in-windows' believes the window was handled and
  ;; leaves the dying buffer in it.  kill-buffer's C safety net then
  ;; shows whatever buffer comes first, visible or not.  Catch that
  ;; case before the C code does: collapse the split when the window is
  ;; not the last one on its frame, otherwise show a scratch buffer
  ;; that is not already on screen.
  (defun mr-x/hidden-scratch-buffer ()
    "Return `*scratch*', or a fresh scratch buffer if that one is on screen.
Checks every frame, since the caller may be acting on a frame that is
not visible yet."
    (let ((scratch (get-scratch-buffer-create)))
      (if (get-buffer-window scratch t)
          (with-current-buffer (generate-new-buffer "*scratch*")
            (funcall initial-major-mode)
            (current-buffer))
        scratch)))

  (defun mr-x/replace-buffer-in-windows-no-duplicates (&optional buffer-or-name)
    "After `replace-buffer-in-windows', fix windows still on BUFFER-OR-NAME."
    (let ((buffer (window-normalize-buffer buffer-or-name)))
      (dolist (window (get-buffer-window-list buffer 'nomini t))
        (if (eq (window-deletable-p window) t)
            (delete-window window)
          (set-window-dedicated-p window nil)
          (set-window-buffer window (mr-x/hidden-scratch-buffer))))))

  (advice-add 'replace-buffer-in-windows :after
              #'mr-x/replace-buffer-in-windows-no-duplicates)

  ;; File links and ordinary buffer navigation from the pane reuse the
  ;; work area, or create it beside a full-frame conversation.  Specific
  ;; popup/conversation rules retain priority over this fallback.
  (autoload 'major-pane-work-buffer-p "major-pane")
  (autoload 'major-pane-display-work-buffer-action "major-pane")
  (add-to-list 'display-buffer-alist
               '(major-pane-work-buffer-p
                 (major-pane-display-work-buffer-action)) t)
...
Falls back to a one-liner if fastfetch isn't installed."
  ...
  ;; Every frame lands on the fetch splash, main and secondary alike.
  ;; Secondary frames used to get *scratch*, but *scratch* is a popper
  ;; popup and `switch-to-buffer-obey-display-actions' routes the
  ;; server's initial switch-to-buffer through display-buffer-alist: the
  ;; scratch landed in a bottom side window and the frame's main window
  ;; kept whatever buffer make-frame inherited from the old selected
  ;; window (the agent pane, typically).  The splash is not a popup, so
  ;; it takes the main window as intended.
  (setq initial-buffer-choice #'mr-x/fetch-buffer)
    ...
    (with-eval-after-load 'general
      ...
      (mr-x/leader-def
        ...
        "v !" '(mr-x/vterm-run :wk "run command in popup")
...
agent-shell, otherwise pop to it. Honours viewport interaction."
      ...
      (defun mr-x/agent-shell-clone ()
        "Start a fresh shell with the current shell's agent, model, and directory.
This creates an independent session; it does not fork or copy history.
Display uses agent-shell's normal path so major-pane controls placement."
        (interactive)
        (require 'agent-shell)
      ...
      ;;   models: "default" (Opus 5.5 1M) "opus[1m]" "fable[1m]"
      ;;           "sonnet" "haiku"
      ;;   modes:  "auto" "default" "acceptEdits" "plan"
      ;;           "bypassPermissions"
      ;; Codex IDs (from a live codex-acp session).  GOTCHA: the session's
      ;; :models list shows suffixed ids ("gpt-5.6-sol[high]") but those are
...
prepended, so they shadow any inherited ANTHROPIC_* values."
      ...
      ;; Codex accounts.  codex-acp spawns the real `codex app-server' with
      ;; its own environment passed through, and Codex keeps its login in
      ;; $CODEX_HOME/auth.json, so pointing CODEX_HOME at another directory
      ;; runs a Codex preset on another ChatGPT login.  ~/.codex-b symlinks
      ;; config.toml, hooks.json, AGENTS.md, skills and plugins back to
      ;; ~/.codex, so only the account differs.  Log one in once with
      ;;   CODEX_HOME=~/.codex-b codex login
      ;; Every Codex preset asks which account to use at spawn time (RET =
      ;; the first entry).  The phone spawn path stays on the first entry.
      (defvar mr-x/agent-shell-codex-accounts
        '(("main" . nil)
          ("B" . "~/.codex-b"))
        "Codex accounts as (NAME . CODEX-HOME).
A nil home means Codex's default ~/.codex.  The first entry is the
default and the only one the phone spawn path uses.")

      (defun mr-x/agent-shell--codex-preset-p (preset)
        "Whether PRESET (a `mr-x/agent-shell-presets' tuple) spawns Codex."
        (eq (nth 4 preset) 'agent-shell-openai-make-codex-config))

      (defun mr-x/agent-shell--read-codex-account ()
        "Ask which of `mr-x/agent-shell-codex-accounts' to use; RET = the first."
        (let* ((accounts mr-x/agent-shell-codex-accounts)
               (prompt (concat (propertize "Account" 'face 'bold) "  "
                               (string-join
                                (seq-map-indexed (lambda (pair i)
                                                   (concat (propertize (number-to-string (1+ i))
                                                                       'face 'bold)
                                                           " " (car pair)))
                                                 accounts)
                                "   ")
                               (propertize "   RET = " 'face 'shadow)
                               (propertize (car (car accounts)) 'face 'shadow)
                               ": "))
               (keys (number-sequence ?1 (+ ?0 (length accounts))))
               (char (read-char-choice prompt (cons ?\r keys))))
          (if (eq char ?\r) (car accounts) (nth (- char ?1) accounts))))

      (defun mr-x/agent-shell--config-for-codex-account (config account)
        "Return CONFIG pointed at Codex ACCOUNT, a (NAME . CODEX-HOME) pair.
An account without a home returns CONFIG untouched.  Any other gets a
copy whose client-maker prepends CODEX_HOME so the child `codex
app-server' reads that home, and whose buffer carries the account name."
        (if (null (cdr account))
            config
          (let ((config (copy-alist config))
                (home (expand-file-name (cdr account)))
                (name (concat "Codex " (car account))))
            (setf (alist-get :mode-line-name config) name)
            (setf (alist-get :buffer-name config) name)
            (setf (alist-get :client-maker config)
                  (lambda (buffer)
                    (agent-shell--make-acp-client
                     :command (car agent-shell-openai-codex-acp-command)
                     :command-params (cdr agent-shell-openai-codex-acp-command)
                     :environment-variables
                     (append (list (concat "CODEX_HOME=" home) "OPENAI_API_KEY=")
      ...
      (defvar mr-x/agent-shell-presets
        '((?f "Fable 5.1 · Full" "fable[1m]"           "bypassPermissions")
          ...
          (?c "Sol 5.6 · Full"   "gpt-5.6-sol"         "agent-full-access"
              agent-shell-openai-make-codex-config "high")
          (?x "Sol 5.6 Max · Full" "gpt-5.6-sol"       "agent-full-access"
              agent-shell-openai-make-codex-config "max")
          (?O "Luna 5.6 · Build" "openai/gpt-5.6-luna" "build"
              agent-shell-opencode-make-agent-config)
          (?g "GLM 5.3 Flash · Full" "openrouter/z-ai/glm-5.3-flash" "bypass"
              agent-shell-opencode-make-agent-config)
...
session exists, retrying TRIES times (the session id lands seconds
after spawn, same dance as `mr-x/agent-spawn--send-when-ready')."
            ...
            (if (map-nested-elt agent-shell--state '(:session :id))
              ...
              (if (> tries 0)
                  (run-at-time 1 nil #'mr-x/agent-shell--set-effort-when-ready
                               buf effort (1- tries))
                (message "agent preset: %s never became ready for effort set"
                         (buffer-name buf)))))))

      (defun mr-x/agent-shell--config-with (model-id mode-id &optional config-fn)
        "Copy the preferred agent config, overriding model + session mode.
When CONFIG-FN is non-nil, build the base config by calling it instead
of resolving the preferred config — that's how a preset targets a
different agent (Codex) entirely.  Returns a fresh config alist so the
original is left untouched."
        (let ((config (copy-alist
                       (or (and config-fn (funcall config-fn))
      ...
      (defvar mr-x/agent-shell--preset-vendors
        '((claude   "Claude"   orange)
          (codex    "Codex"    green)
          (opencode "OpenCode" lime)
          (deepseek "DeepSeek" gold)
          (other    "Other"    cream))
        "Preset rows: (VENDOR LABEL PALETTE-COLOR), in display order.")

      (defun mr-x/agent-shell--preset-vendor (preset)
        "Vendor symbol for PRESET, from its config constructor (nil = Claude)."
        (pcase (nth 4 preset)
          ('nil 'claude)
...
and the SPC c m keys cannot drift apart."
      ...
      (defun mr-x/agent-shell--preset-mode-color (word)
        ...
        (pcase word
          ("full" 'red)
          ((or "accept edits" "auto") 'gold)
          ("build" 'lime)
          (_ 'mint)))

      (defun mr-x/agent-shell--preset-entry (preset vendor-label)
        "Render one PRESET as \"k Model mode\", dropping VENDOR-LABEL from the model."
        (let* ((label (nth 1 preset))
               (model (string-trim (replace-regexp-in-string " · .*\\'" "" label)))
               (model (if (and (string-prefix-p vendor-label model)
                               (> (length model) (length vendor-label)))
                          (string-trim (substring model (length vendor-label)))
                        model))
               (word (mr-x/agent-shell--preset-mode-word (nth 3 preset))))
          (concat (propertize (string (nth 0 preset)) 'face 'bold)
                  " " model " "
      ...
      (defun mr-x/agent-shell--preset-prompt (question)
        ...
        (let* ((width (apply #'max (mapcar (lambda (v) (length (nth 1 v)))
                                           mr-x/agent-shell--preset-vendors)))
               (rows
                (delq nil
                      (mapcar
                       (lambda (vendor)
                         (let ((presets (seq-filter
                                         (lambda (p) (eq (mr-x/agent-shell--preset-vendor p)
                                                         (car vendor)))
                                         mr-x/agent-shell-presets)))
                           (when presets
                             (concat
                              (propertize (format (format "%%-%ds" width) (nth 1 vendor))
                                          'face `(:foreground ,(mr-x/color (nth 2 vendor))
                                                  :weight bold))
                              "  "
                              (mapconcat (lambda (p) (mr-x/agent-shell--preset-entry p (nth 1 vendor)))
                                         presets "   ")
                              (when (and (eq (car vendor) 'codex)
                                         (> (length mr-x/agent-shell-codex-accounts) 1))
                                (propertize "   asks account" 'face 'shadow))))))
                       mr-x/agent-shell--preset-vendors))))
...
if it matches no preset."
        (let ((char (or char
                        (read-char-choice
                         (mr-x/agent-shell--preset-prompt "Preset: ")
...
callers can `let'-bind it to launch the preset in another project."
        (require 'agent-shell)
        (let* ((account (when (mr-x/agent-shell--codex-preset-p preset)
                          (mr-x/agent-shell--read-codex-account)))
               (config (mr-x/agent-shell--config-with (nth 2 preset) (nth 3 preset)
                                                      (nth 4 preset)))
               (config (if account
                           (mr-x/agent-shell--config-for-codex-account config account)
                         config))
               (shell-buffer (agent-shell--start :config config
                                                 :no-focus t
                                                 :new-session t)))
          ...
          (message "Agent preset: %s%s" (nth 1 preset)
                   (if (cdr account) (format " (%s)" (car account)) ""))))
...
default config."
        (let* ((chars (mapcar #'car mr-x/agent-shell-presets))
               (char (read-char-choice
                      (mr-x/agent-shell--preset-prompt "Preset (RET = Claude default): ")
                      (append chars (list ?\r ?\n ?\s)))))
          (unless (memq char '(?\r ?\n ?\s))
            (or (assq char mr-x/agent-shell-presets)
                (user-error "No preset for %c" char)))))
...
the `?c' preset from `mr-x/agent-shell-presets'."
      ...
      (mr-x/leader-def
        ...
        "c / s s" '(agent-recall-search-summaries :wk "Summaries")
...
OpenCode.  Codex Plan lives in `collaboration_mode', handled here too."
      ...
      (mr-x/leader-def
        ...
        "& m" '(major-pane-set-placement :wk "Pane placement")
...
Handlers in mr-x/--restore-buffer-from-spec must stay in sync with this."
    (with-current-buffer buffer
      (cond
       ...
       ((and (derived-mode-p 'agent-shell-mode) (eq (bound-and-true-p major-pane--excluded) t))
        '(excluded-agent))
       ((derived-mode-p 'agent-shell-mode)
        (when (fboundp 'agent-shell--state)
          (let ((session-id (ignore-errors
            ...
            (when session-id
              (append (list 'agent-shell :dir default-directory)
                      (major-pane-workspace--entry buffer))))))
  ...
  (defun mr-x/--serialize-window-tree (window)
    ...
    (if (window-live-p window)
        (list :type 'leaf
              ...
              :pane-mode (and (window-parameter window 'major-pane)
                              (major-pane-state-mode major-pane--state)))
  ...
  (defun mr-x/--session-frame-eligible-p (frame)
    "Whether FRAME is an ordinary visible top-level graphical editor frame.
The daemon's bootstrap terminal frame, tty client frames, iconified
frames, child frames (posframes and other popups), and minibuffer-only
frames are excluded.  This one predicate decides both the session
snapshot and the run-scoped crash capture, so they always agree."
  ...
  (defun mr-x/--session-eligible-frames ()
    "Frames the session snapshot covers, in `frame-list' order."
    (filtered-frame-list #'mr-x/--session-frame-eligible-p))

  (defun mr-x/--session-frame-key (frame)
    "Return FRAME's stable restore key, assigning one on first use.
The key lives in a frame parameter so reconstruction can reapply it and
yabai placement can join on it instead of on titles or creation order."
    (or (frame-parameter frame 'mr-x/restore-key)
        (let ((key (format "frame-%s"
                           (substring (md5 (format "%s-%s-%s" (emacs-pid)
                                                   (float-time) (random)))
                                      0 12))))
          (set-frame-parameter frame 'mr-x/restore-key key)
          key)))

  (defun mr-x/--session-frame-keys ()
    "Restore keys of the currently eligible frames."
    (mapcar #'mr-x/--session-frame-key (mr-x/--session-eligible-frames)))

  (defun mr-x/--session-frames ()
    "Serialize every eligible frame: restore key, geometry, and window tree."
    (mapcar (lambda (f)
              (list :restore-key (mr-x/--session-frame-key f)
                    ...
                    :window-tree (mr-x/--serialize-window-tree
                                  (frame-root-window f))))
            (mr-x/--session-eligible-frames)))

  (defun mr-x/save-session-state ()
    "Save conversations and matching frame state through the shared capture path."
    (interactive)
    (when major-pane-workspace-inhibit-save (user-error "Workspace restoration is still running"))
    (let ((entries (major-pane-workspace--collect)))
      (mr-x/session-capture entries)
      (major-pane-workspace--save-history entries)
      (message "Session saved: %d frames" (length (car mr-x/session-capture-last)))))
...
Signal handler errors so the frame transaction can roll back and keep evidence.
Nil means no handler could reconstruct the buffer; the caller must reject it."
    (condition-case err
        (pcase (car spec)
          ('vterm
           (when (require 'vterm nil t)
             (let ((default-directory (or (plist-get (cdr spec) :dir)
                                          default-directory)))
               (let ((buffer (save-window-excursion (vterm (generate-new-buffer-name "*restored-vterm*")))))
                 (mr-x/crash-workspace-own-buffer buffer)))))
          ('excluded-agent
           (let* ((name "*Recovery: excluded background agent*")
                  (existing (get-buffer name))
                  (buffer (get-buffer-create name)))
             (unless existing
               (with-current-buffer buffer
                 (insert "This window showed an excluded background agent. It was intentionally not restarted.\n")
                 (special-mode))
               (mr-x/crash-workspace-own-buffer buffer))
             buffer))
          ('agent-shell
           (mr-x/crash-workspace-buffer (plist-get (cdr spec) :agent)
                                        (plist-get (cdr spec) :session-id)))
          ('project-dashboard
          ...
          (_ nil))
      (error
       (error "Restore handler failed for %S: %s"
              (car spec) (error-message-string err)))))

  (defun mr-x/--restore-window-tree (window tree)
    "Restore TREE into WINDOW, splitting as needed.
Each leaf must resolve its saved file, restore-spec, or existing buffer name.
File and handler failures signal rather than fall back to another buffer, so
the frame transaction cannot consume a bundle after an incomplete restore."
    (if (eq (plist-get tree :type) 'leaf)
               ...
               (buf (cond
                     (file
                      (unless (file-exists-p file)
                        (error "Cannot restore buffer %S: file missing: %s" buf-name file))
                      (find-file-noselect file))
                     (spec (mr-x/--restore-buffer-from-spec spec))
                     (t (get-buffer buf-name)))))
          (unless (buffer-live-p buf)
            (error "Cannot restore buffer %S%s"
                   buf-name (if spec (format ": handler %S returned no live buffer" (car spec))
                              ": no saved file, restore handler, or live buffer")))
          (set-window-buffer window buf)
          (set-window-point window (or (plist-get tree :point) 1))
          (set-window-start window (or (plist-get tree :start) 1))
          (set-window-dedicated-p window (plist-get tree :dedicated))
          (when (plist-get tree :pane-mode)
  ...
  (defvar mr-x/session-restore-result nil)
  (defvar mr-x/session-restore-operation nil)

  (defun mr-x/restore-session-state ()
    "Asynchronously restore the saved frames with verified agent sessions.
The legacy restart script polls `mr-x/session-restore-result' before placement."
    (interactive)
    (unless (file-exists-p mr-x/session-file) (user-error "No session state file found"))
    (when mr-x/crash-workspace-current (user-error "A workspace restore is already running"))
    (let ((session (mr-x/crash-restore-legacy-session
                    (mr-x/crash-capture--read mr-x/session-file))))
      (setq mr-x/session-restore-result '(:status pending))
      (condition-case err
          (setq mr-x/session-restore-operation
                (mr-x/crash-workspace-start
                 (format "manual-restore-%s" (float-time)) nil session #'mr-x/--crash-replay-tree
                 (lambda (result)
                   (setq mr-x/session-restore-result result)
                   (message "Session restore: %s" (plist-get result :status))
                   (major-pane-workspace-request-save))))
        ((error quit)
         (setq mr-x/session-restore-result
               (list :status 'failed :errors (list (error-message-string err))))))
      (plist-get mr-x/session-restore-result :status)))

  (defun mr-x/cancel-session-restore ()
    "Cancel only the active manual session restore."
    (interactive)
    (when mr-x/session-restore-operation
      (mr-x/crash-workspace-cancel mr-x/session-restore-operation)))
  ...
  (defvar mr-x/session-placement-last nil)

  (defun mr-x/--session-layout-signature (frames)
    "Return FRAMES without volatile cursor and scroll positions."
    (cl-labels ((tree (node)
                 (if (eq (plist-get node :type) 'leaf)
                     (cl-loop for (key value) on node by #'cddr
                              unless (memq key '(:point :start)) append (list key value))
                   (plist-put (copy-sequence node) :children
                              (mapcar #'tree (plist-get node :children))))))
      (mapcar (lambda (frame)
                (plist-put (copy-sequence frame) :window-tree
                           (tree (plist-get frame :window-tree)))) frames)))

  (defun mr-x/--session-save-legacy-placement (frames)
    "Refresh the compatibility yabai mirror on structural layout changes.
This remains separate from validated crash-bundle placement."
    (let ((signature (mr-x/--session-layout-signature frames)))
      (when (and (not noninteractive) frames
                 (not (equal signature mr-x/session-placement-last))
                 (not (get-process "yabai-autosave"))
                 (file-exists-p mr-x/--restart-restore-script))
        (let* ((process-environment
                (cons (concat "YABAI_STATE=" mr-x/yabai-state-file) process-environment))
               (process (start-process "yabai-autosave" nil mr-x/--restart-restore-script "save-yabai")))
          (setq mr-x/session-placement-last signature)
          (set-process-query-on-exit-flag process nil)
          (set-process-sentinel
           process (lambda (proc _event)
                     (when (memq (process-status proc) '(exit signal))
                       (unless (zerop (process-exit-status proc))
                         (message "Legacy Space snapshot failed: exit %d" (process-exit-status proc)))
                       (major-pane-workspace-request-save 0.25))))))))

  (defvar mr-x/session-capture-last nil
    "Last successfully published (frames workspace) pair in this daemon.")

  (defun mr-x/session-capture (entries)
    "Publish ENTRIES and one matching frame collection after a workspace event."
    (let* ((frames (mr-x/--session-frames))
           (pair (list frames entries)))
      (unless (equal pair mr-x/session-capture-last)
        (when (or frames entries mr-x/session-capture-last)
          (when (bound-and-true-p mr-x/crash-runtime--identity)
            (let ((result (mr-x/crash-runtime-capture
                           (lambda () frames) nil #'mr-x/--session-frame-keys
                           (lambda () entries))))
              (unless (eq (plist-get result :status) 'committed)
                (error "Crash capture was not committed: %s"
                       (or mr-x/crash-runtime--last-error result)))))
          ;; The legacy mirror and picker history use this collection too.
          (make-directory (file-name-directory mr-x/session-file) t)
          (mr-x/crash-capture--write mr-x/session-file frames)
          (setq mr-x/session-capture-last (copy-tree pair))))
      (mr-x/--session-save-legacy-placement frames)))

  (defun mr-x/session-autosave ()
    "Explicitly flush the event-driven workspace capture."
    (major-pane-workspace--flush))

  (setq major-pane-workspace-capture-function #'mr-x/session-capture)
...
records of a bundle, or the tail of a legacy stderr snapshot."
  ...
  (defun mr-x/--crash-frames-with-keys (keys &optional owner)
    "Live frames whose restore key is one of KEYS."
    (cl-remove-if-not
     (lambda (f) (and (member (frame-parameter f 'mr-x/restore-key) keys)
                      (or (null owner) (equal (frame-parameter f 'mr-x/recovery-owner) owner))))
     (frame-list)))
  ...
  (defvar mr-x/--crash-restore-operation nil)

  (defun mr-x/--crash-restore-missing (result)
    "One line naming RESULT's conversations that did not resume, or nil."
    (when-let ((failed (plist-get result :failed)))
      (mapconcat (lambda (f)
                   (format "%s (%s %s): %s"
                           (or (plist-get f :label) "unlabeled") (plist-get f :agent)
                           (plist-get f :session-id) (plist-get f :error)))
                 failed "; ")))

  (defun mr-x/--crash-restore-done (info splash-frame result)
    "Publish the settled RESULT without consuming incomplete recovery evidence.
Frames that came back stay up even when some conversations did not: those
windows hold placeholders, the bundle returns to `pending-frames' with the
missing conversations in :last-error, and nothing is consumed, so they can
still be recovered from the evidence."
    (let* ((directory (plist-get info :directory))
           (status (mr-x/crash-bundle-status directory))
           (leaked (append (plist-get result :leaked) (plist-get result :agent-leaks)))
           (missing (mr-x/--crash-restore-missing result))
           (restored-frames (mapcar (lambda (f) (list :restore-key (plist-get f :restore-key)
                                                     :window-id nil))
                                    (plist-get result :frames))))
      (unwind-protect
          (cond
           ((not (eq (plist-get result :status) 'restored))
            (mr-x/crash-bundle-transition
             directory 'restoring-frames (if leaked 'cleanup-pending 'pending-frames)
             :last-error (format "%S" (plist-get result :errors)) :residual-resources leaked)
            (mr-x/crash-recovery)
            (message "Recovery failed; evidence kept: %S" (plist-get result :errors)))
           (missing
            (mr-x/crash-bundle-transition
             directory 'restoring-frames 'pending-frames
             :restored-frames restored-frames
             :last-error (format "Not resumed: %s" missing))
            (when (frame-live-p splash-frame)
              (lower-frame splash-frame))
            (message "Workspace restored: %d frames; %d conversation(s) not resumed, evidence kept"
                     (length (plist-get result :frames)) (length (plist-get result :failed))))
           (t
            (mr-x/crash-bundle-transition
             directory 'restoring-frames 'pending-yabai :restored-frames restored-frames)
            (if (plist-get status :yabai-required)
                (message "Conversations and frames restored; Space placement remains pending")
              (mr-x/crash-bundle-transition directory 'pending-yabai 'complete)
              (mr-x/crash-bundle-consume (mr-x/--crash-store) (plist-get info :id))
              (mr-x/--crash-after-consume splash-frame)
              (message "Workspace restored: %d frames" (length (plist-get result :frames))))))
        (setq mr-x/--crash-restore-active nil mr-x/--crash-restore-operation nil
              major-pane-workspace-inhibit-save nil)
        (major-pane-workspace-request-save))))

  (defun mr-x/--crash-restore-bundle (info)
    "Restore verified conversations before replaying the bundle's frames."
    (let* ((id (plist-get info :id)) (directory (plist-get info :directory))
           (status (mr-x/crash-bundle-status directory))
           (phase (plist-get status :phase))
           (owner (cons id (plist-get status :attempt)))
           (splash-frame (selected-frame)))
      (when (or mr-x/--crash-restore-active mr-x/crash-workspace-current)
        (user-error "A workspace recovery is already running"))
      (pcase phase
        ((or 'cleanup-pending 'restoring-frames)
         (let* ((keys (mapcar (lambda (fd) (plist-get fd :restore-key))
                              (mr-x/crash-capture--read (plist-get info :session))))
                (agent-leaks (mr-x/crash-workspace-cleanup owner)))
           (dolist (f (mr-x/--crash-frames-with-keys keys owner))
             (ignore-errors (delete-frame f t)))
           (let ((left (append agent-leaks
                               (mapcar (lambda (f) (frame-parameter f 'mr-x/restore-key))
                                       (mr-x/--crash-frames-with-keys keys owner)))))
             (mr-x/crash-bundle-transition
              directory phase (if left 'cleanup-pending 'pending-frames)
              :residual-resources left :last-error "Interrupted attempt cleanup")
             (mr-x/crash-recovery)
             (message (if left "Cleanup incomplete; retry cleanup" "Cleanup complete; retry recovery")))))
        ('pending-frames
         (let* ((data (mr-x/crash-workspace-read-bundle directory))
                (session (car data)) (workspace (cadr data))
                (attempt (1+ (plist-get status :attempt))))
           ;; Fail before creating resources or changing status on bad evidence.
           (mr-x/crash-workspace--entries workspace session)
           (mr-x/crash-bundle-transition directory 'pending-frames 'restoring-frames
                                         ...
                                         :attempt attempt)
           (setq mr-x/--crash-restore-active id major-pane-workspace-inhibit-save t)
           (condition-case err
               (let ((operation
                      (mr-x/crash-workspace-start
                       (cons id attempt) workspace session #'mr-x/--crash-replay-tree
                       (lambda (result) (mr-x/--crash-restore-done info splash-frame result)))))
                 (when mr-x/--crash-restore-active
                   (setq mr-x/--crash-restore-operation operation)))
             ((error quit)
              (mr-x/--crash-restore-done
               info splash-frame (list :status 'failed :errors (list (error-message-string err))))))))
        (_ (user-error "Bundle %s is %s; restore is not available" id phase)))))
...
made is deleted and the evidence stays; the review shows why."
  ...
  (when (daemonp)
    (when (timerp mr-x/session-autosave-timer)
      (cancel-timer mr-x/session-autosave-timer))
    (setq mr-x/session-autosave-timer nil))
...
Pasteable into Finder, Slack, Mail, etc.  (\"w\" copies the path as text.)"
    ...
    ;; Search the target text, then let the user choose an actual index entry.
    (add-to-list 'embark-target-injection-hooks
                 '(devdocs-lookup embark--allow-edit))
  ...
  (use-package diff-hl
    :ensure t
    :init
    ;; Remote dired: diff-hl runs git over the TRAMP connection per dir.
    (defun mr-x/diff-hl-dired-maybe ()
      (unless (file-remote-p default-directory)
        (diff-hl-dired-mode 1)))
    :hook ((prog-mode . diff-hl-mode)
           (text-mode . diff-hl-mode)
           (dired-mode . mr-x/diff-hl-dired-maybe)
...
"
  ...
  ;; Highlight source syntax in raw patches (including Forgejo review diffs),
  ;; falling back to the hunk when full file/revision context is unavailable.
  (setq diff-font-lock-syntax 'hunk-also)

  ;; Gruvbox's inverse-video refinement makes large replacements neon.
  ;; Keep word-change backgrounds subdued and let syntax supply text colors.
  (custom-set-faces
   '(diff-refine-added ((t (:foreground unspecified :background "#3d5c3d"
                            :inverse-video nil))))
   '(diff-refine-removed ((t (:foreground unspecified :background "#5c3030"
                              :inverse-video nil)))))

  ;; Native Forgejo reviews.  GNU maintains the source branches; give each
  ;; branch its own Elpaca checkout because they share the same remote URL.
  (use-package keymap-popup
    :ensure (:repo ("https://git.savannah.gnu.org/git/elpa/gnu.git" . "keymap-popup")
             :branch "externals/keymap-popup" :depth 1))

  (use-package forgejo
    :ensure (:repo ("https://git.savannah.gnu.org/git/elpa/gnu.git" . "forgejo")
             :branch "externals/forgejo" :depth 1
...
a diagram when only particular nodes should change."
    ...
    (defcustom mr-x/markdown-mermaid-flowchart-options
      '((htmlLabels . t)
        (wrappingWidth . 340)
        (padding . 14)
        (nodeSpacing . 45)
        ...
        (curve . "basis")
        (useMaxWidth . t))
      "Mermaid `flowchart' layout options.
Mermaid's stock `wrappingWidth' of 200 pixels wraps every label into a
ragged column roughly twenty monospace characters wide, so it is raised
here.  Diagram-local `%%{init: {\"flowchart\": {...}}}%%' still wins."
      :type '(alist :key-type symbol :value-type sexp)
      :group 'markdown-xwidget)

    (defcustom mr-x/markdown-mermaid-theme-css
      (concat
       ;; Mermaid measures HTML labels in a probe element that inherits the
       ;; preview's GitHub font stack, then renders them in `fontFamily'.
       ;; When the two disagree the computed node boxes are too tall or too
       ;; short (labels visibly clip).  Pin one font everywhere labels live.
       ".nodeLabel, .nodeLabel p, .edgeLabel, .edgeLabel p,"
       " .cluster-label, .cluster-label p, .label, .labelText,"
       " foreignObject div, foreignObject span {\n"
       "  font-family: " mr-x/markdown-mermaid-font-family " !important;\n"
       ...
       ;; Mermaid sizes a cluster's title band for one line, so a wrapped
       ;; subgraph title is drawn clipped through the cluster border.  Keep
       ;; those titles on a single line; nodes still wrap normally.
       ".cluster-label, .cluster-label p, .cluster-label div,"
       " .cluster-label .nodeLabel {\n"
       "  white-space: nowrap !important;\n"
       "  text-align: center !important;\n"
       "}\n"
       ".nodeLabel { font-weight: 600; }\n"
       ;; Multi-line labels read as prose when centred.  Left-align node
       ;; bodies; edge labels are short and stay centred.
       ".nodeLabel, .nodeLabel p { text-align: left !important; }\n"
       ".nodeLabel p { margin: 0 !important; }\n"
       ".nodeLabel p + p { margin-top: 3px !important; }\n"
       ".edgeLabel, .edgeLabel p { text-align: center !important; }\n"
       ;; A Markdown-string label writes one <p> per line, so the first line
       ;; of a multi-line node becomes its heading for free.
       ".nodeLabel p:first-child:not(:only-child) {\n"
       "  color: #fabd2f !important;\n"
       "  font-weight: 800 !important;\n"
       "  letter-spacing: 0.01em !important;\n"
       "}\n"
       ;; Card classes.  Declared once here so any diagram can just write
       ;; `:::card-accent' without repeating a classDef palette.
       ".node.card-accent > rect, .node.card-accent > polygon,"
       " .node.card-accent > path {\n"
       "  fill: #3c3836 !important;\n"
       "  stroke: #fe8019 !important;\n"
       "  stroke-width: 2px !important;\n"
       "}\n"
       ".node.card-accent .nodeLabel p:first-child { color: #fe8019 !important; }\n"
       ".node.card-muted > rect, .node.card-muted > polygon,"
       " .node.card-muted > path {\n"
       "  fill: #32302f !important;\n"
       "  stroke: #665c54 !important;\n"
       "  stroke-width: 1.5px !important;\n"
       "}\n"
       ".node.card-muted .nodeLabel, .node.card-muted .nodeLabel p { color: #a89984 !important; }\n"
       ".node.card-muted .nodeLabel p:first-child { color: #bdae93 !important; }\n"
       ".node.card-warn > rect, .node.card-warn > polygon,"
       " .node.card-warn > path {\n"
       "  fill: #422b21 !important;\n"
       "  stroke: #fb4934 !important;\n"
       "  stroke-width: 2px !important;\n"
       "}\n"
       ".node.card-warn .nodeLabel p:first-child { color: #fb4934 !important; }\n"
       ".node.card-ghost > rect, .node.card-ghost > polygon,"
       " .node.card-ghost > path {\n"
       "  fill: transparent !important;\n"
       "  stroke: #665c54 !important;\n"
       "  stroke-width: 1.5px !important;\n"
       "  stroke-dasharray: 5 4 !important;\n"
       "}\n"
       ".node.card-ghost .nodeLabel, .node.card-ghost .nodeLabel p { color: #928374 !important; }")
      "Extra Mermaid CSS for diagram titles, group headings, and labels.
Also defines the site-wide `card-accent', `card-muted', `card-warn', and
`card-ghost' node classes, applied in a diagram with `:::card-accent'."
      :type 'string
      :group 'markdown-xwidget)
    ...
    (defun mr-x/markdown-mermaid-config ()
      ...
      (let ((theme-variables (copy-tree mr-x/markdown-mermaid-theme-variables)))
        ...
        `((theme . ,markdown-xwidget-mermaid-theme)
          ...
          (startOnLoad . t)
          (flowchart . ,mr-x/markdown-mermaid-flowchart-options)
          (themeVariables . ,theme-variables)
          (themeCSS . ,mr-x/markdown-mermaid-theme-css))))

    ;; The phone renders the same diagrams with the same engine, so it
    ;; reads this same config rather than keeping a second copy of the
    ;; palette.  Exporting here means the JSON is refreshed whenever
    ;; markdown-xwidget loads; `syzygy-export-mermaid-config' is also a
    ;; command for a manual refresh after a Customize edit.
    (when (require 'syzygy-mermaid nil t)
...
markdown-xwidget discover `<code class=\"mermaid\">...'."
    ...
    (defun mr-x/markdown-mermaid-export-source (source)
      "Export Mermaid SOURCE as a standalone themed inspector page."
      (when (string-empty-p (string-trim source))
        (user-error "Mermaid source is empty"))
      (make-directory mr-x/markdown-preview-directory t)
      (let* ((digest (substring (secure-hash 'sha256 source) 0 16))
             (output
              (expand-file-name
               (format "agent-shell-mermaid-%s.html" digest)
               mr-x/markdown-preview-directory))
             (github-theme
              (markdown-xwidget-github-css-path markdown-xwidget-github-theme))
             (code-block-theme
              (markdown-xwidget-highlightjs-css-path
        ...
        (with-temp-buffer
          ...
          (unless (string-suffix-p "\n" source)
            (insert "\n"))
          (insert "```\n")
          (setq-local markdown-css-paths
                      (list github-theme code-block-theme
                            mr-x/markdown-gruvbox-css-file))
          (setq-local markdown-command
                      (or markdown-xwidget-command markdown-command))
          ;; `markdown-standalone' switches to its output buffer before it
          ;; reads this variable, so keep the header dynamically bound rather
          ;; than buffer-local to this synthetic source buffer.
          (let ((markdown-xhtml-header-content
                 (mr-x/markdown-preview-header-html)))
            ;; Avoid making `markdown-export' save this buffer as source.
            (set-buffer-modified-p nil)
            (markdown-export output)))
        (mr-x/markdown-normalize-pandoc-mermaid-file output)))
...
Diagram-local frontmatter with its own `look' remains authoritative."
      ...
      (let ((next-look (if (equal mr-x/markdown-mermaid-look "classic")
        ...
        (when (fboundp 'syzygy-export-mermaid-config)
          (syzygy-export-mermaid-config))
...
and that flag are killed, so the main browser is never touched."
    ...
    (defun mr-x/markdown-mermaid-launch-chrome-uri (uri mode index)
      "Open diagram INDEX from URI in Chrome according to MODE.
MODE is `tab' for a normal browser tab or `app' for a dedicated window."
      (let* ((base-uri (replace-regexp-in-string "[?#].*\\'" "" uri))
    ...
    (defun mr-x/markdown-mermaid-launch-chrome (xwidget mode index)
      ...
      (mr-x/markdown-mermaid-launch-chrome-uri
       (xwidget-webkit-uri xwidget) mode index))

    (defun mr-x/markdown-mermaid-pop-out (source)
      "Export Mermaid SOURCE and open it directly in the app-window viewer."
      (let ((output (mr-x/markdown-mermaid-export-source source)))
        (mr-x/markdown-mermaid-launch-chrome-uri
         (concat "file://" (expand-file-name output)) 'app 0)
        output))
...
;; CommonJS files need the same JavaScript highlighting in buffers and diffs.
...
;; Tree-sitter grammar sources for TS/TSX. Both grammars live in
;; subdirectories of the same repo, so each needs an explicit source
;; dir (4th element) — a bare URL fails to build. With treesit-auto's
;; 'prompt setting, opening a .ts/.tsx file offers to compile these;
;; or run M-x treesit-install-language-grammar manually.
