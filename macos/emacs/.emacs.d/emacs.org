...
** Elpaca
Boot status comes first: the sketchybar cacodemon shows what the daemon is
doing while Elpaca works through its queue (see =lisp/mr-x-boot-status.el=).
...
~require~'d from the org ~:config~ block below. ~lisp/syzygy/~ holds the
cross-device conversation-continuity package (resync/live/handoff).
#+begin_src emacs-lisp
...
** Strudel music
Edit Strudel in a normal JavaScript buffer.  Run ~M-x mr-x/strudel-play~ to
start the background WebKit audio engine and evaluate the current buffer.
A fresh audio session may display a one-time click-to-unlock prompt; its
pane hides automatically after the click.
~M-x mr-x/strudel-update~ sends unsaved edits; saving alone does not update
the music.  ~M-x mr-x/strudel-stop~ stops playback, and
~M-x mr-x/strudel-shutdown~ releases this session's bridge and audio buffer.
~M-x mr-x/strudel-show-audio~ exposes the audio page for visuals or errors.

Requires graphical Emacs with WebKit, Node, Bun, and ~npm install -g
strudex@0.1.0~.  The runtime downloads Strudel JavaScript and samples on first
use.  Implementation: ~lisp/strudel-control.el~.  Bridge logs are in
~*Strudel bridge*~.  No keybindings or save hooks are installed.
#+begin_src emacs-lisp
...
ARCA (the family Nextcloud) is the store for Marx's tasks and calendar; Home
Assistant reads and writes the same collections, so a reminder spoken to
Andromeda, ticked on the iPhone, or typed here is one item. Spec:
home-lab/docs/superpowers/specs/2026-09-05-arca-pim-org-integration-design.md.

One machine syncs. org-caldav keeps its state in the per-machine var dir, and
two machines syncing one server against one Syncthing'd file set is how
duplicates and phantom deletions happen. MrX2 loads this block and never syncs.

Announce time is =DEADLINE=: VTODO =DUE= imports as =DEADLINE= and only
=DEADLINE= exports as =DUE=. A =SCHEDULED=-only TODO never announces.

The server address lives outside this public repo, in
=~/.config/org-caldav/url= (one line, the CalDAV base URL for the user's
calendars). Missing file = org-caldav stays unconfigured and never syncs.
Credentials: =~/.authinfo=, =machine HOST:443 login USER= (a Nextcloud app
password). Nothing in this file.
...
** Dictionary Lookup
Word definitions from the native macOS Dictionary.app data (New Oxford
American — same source Raycast's define uses), offline and instant.
First lookup auto-compiles the tiny =osx-dictionary-cli= helper.
Bound in the =SPC ?= lookup family.

The CLI dumps one smashed-together blob, so =mr-x/osx-dictionary--prettify=
re-typesets the result buffer after every search: headword bolded on its
own line, blank lines between senses/bullets, and dictionary sections
(ORIGIN, PHRASES, …) split out as separated headers.
...
The dashboard's client is buffer-local, so any caller outside
=*Music Assistant*= (emacsclient, the DS4 remote) has to borrow its
buffer before invoking a command.
...
Evil loads before general, on purpose. Every =mr-x/leader-def= binding
carries five evil states, and general defers each one with its own
=eval-after-load 'evil= when evil is not loaded yet: about 2,300
callbacks. Emacs queues those through =after-load-functions= with a
depth sort that grows roughly with the cube of their number, which cost
42 of a 49 second boot (found 2026-09-25). With evil already loaded,
general binds on the spot.
...
*** Jump back across windows

Evil's =C-o= keeps one jump list per window and skips buffers without
a file, so after =SPC c a r= it cannot get back to the file you came
from. =lisp/mr-x-jump-back.el= keeps one history for the whole session
and re-selects the original window. =C-o= goes back through it.
...
*** Send buffers to CALLIOPE (the e-ink tablet)

CALLIOPE's =emx= is a live =emacsclient -t= tty into this daemon — a dumb-glass
frame on the e-ink. It's the sole non-graphic frame, so we find it by
=display-graphic-p=. That frame's mere existence *is* the "emx is running" check:
no frame ⇒ emx isn't launched.

Every send goes through =mr-x/calliope-ensure-frame=: if emx is already running it
runs the payload immediately; otherwise it fires =calliope-wake.sh emx= to launch
emx, polls until the tty frame connects, then runs the payload. The launch is
async so Emacs never blocks on the tailnet round-trip. All bound under =SPC .=
(calliope prefix).

Scrolling is a send too: =SPC . g g= and =SPC . G= move whatever is already on
the e-ink to the top or the bottom, mirroring evil's =gg= / =G=. They act on the
tty frame's own window rather than launching emx, because with nothing on the
panel there is nothing to scroll.

(Note: this is about launching emx, not physically waking the tablet — off its
charger the Boox drops Wi-Fi and can't be reached at all, so it lives on the
charger, screen always on. The "wake" in the script name is incidental; what we
actually rely on is the emx launch.)
...
Workspace and layout events publish the current conversation list and frames.
The legacy =session-state.el= mirror shares that collection; structural layout
changes refresh its compatibility yabai mirror.  When launched through
=emacs-daemon-run.sh=, the daemon commits conversations and frames atomically
in its run directory.  No recurring snapshot timer is needed.
Crash detection lives in the launcher path (=lisp/mr-x-crash-runtime.el=):
each run has its own diagnostics and clean-exit marker, and the next
start turns every unclean run into an immutable bundle under
=crash-state/bundles/= (=lisp/mr-x-crash-bundle.el=).  Nothing restarts
or restores automatically — after a crash you start Emacs like always,
land on the splash (which shows a quiet hint), and run =SPC R= whenever
you're ready.  That renders the newest pending bundle into the splash:
which run crashed, what frames it held, the latest command errors, and
single-key actions.  =r= rebuilds the frames as one attempt that rolls
its own frames back on failure (=lisp/mr-x-crash-restore.el=; yabai
placement is not wired to bundles yet); =d= opens the bundle's full
report; =x= discards it and shows the next pending one.

A flat snapshot left by the pre-bundle checker is still reviewable and
discardable as a legacy entry, but never rewritten.  All of this is
daemon-only: batch runs (tests, tangling) must not start timers.
...
#+end_src
*** Bookmark popup from any app

=Cmd+Ctrl+B= invokes =mr-x/bookmark-popup= through Hammerspoon.
The floating picker uses the running daemon's named bookmarks and current
completion setup.  Choosing a bookmark closes the picker and runs its normal
handler; cancelling returns to the invoking app.  The sandbox is not required.
The matching yabai scratchpad rule keeps its window out of the tiling hook.
...
A thin divider above the live prompt, so the place to type stays easy to
find while a reply streams in above it.  One overlay per shell, re-synced
to the live prompt after every buffer change; hidden when no prompt is live.
...
** Voicelog
The house voice log (=voicelog.andrade-lab.com=) as an Emacs buffer:
browser-style cards, org-style folding, =C-j= / =C-k= card motion, live
refresh every 15 s while visible. Evil motions are untouched; every filter
(who, time frame, origin, search, overheard, unanswered) and action lives in
the =C-c f= transient. Package in =lisp/voicelog.el=.
#+begin_src emacs-lisp
