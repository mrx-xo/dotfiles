                  ...
                  (with-selected-window source-window
                    (let ((anchor (with-current-buffer source
                      ...
                      (posframe-show
                       ...
                       :width (min 72 (- (frame-width) 4))
                       :min-width (min 40 (- (frame-width) 4)) :max-height 24
        ...
        (let ((inhibit-read-only t))
          ...
          (if (not response)
            ...
            (let ((clean-response (mr-x/quick-ask--strip-thinking response)))
              ...
              ;; Header
              (setq-local header-line-format
                          (propertize " q dismiss  c chat  u park  y copy  r again"
                                      'face '(:weight bold)))
              (when mr-x/quick-ask--source-origin
                (insert (propertize
                         (concat (plist-get mr-x/quick-ask--source-origin :label) "\n")
                         'face 'shadow)))
              ;; Question echo
              (insert (propertize question 'face '(:weight bold :foreground "#88c0d0")))
              (insert "\n")
