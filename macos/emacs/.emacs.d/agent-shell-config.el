...
up).  Deduped by session id."
        ...
        (defun mr-x/agent-handoff--read (candidates)
                 ...
                 (annotate
                  (lambda (cand)
                    (let* ((pl (funcall lookup cand))
                           ...
                           (color (or (cdr (assoc mach mr-x/agent-handoff--machine-colors))
                                      "#928374")))
                      (concat
                       "   "
                       (propertize (format "%-10s" mach)
                                   'face `(:foreground ,color :weight bold))
                       (propertize (format "%6s ago" (mr-x/agent-handoff--age
                                                      (plist-get pl :mtime)))
                                   'face 'shadow)))))
                 (newest-first
                  (lambda (cands)
                  ...
                  (completing-read
                   ...
                   (lambda (str pred action)
                     (if (eq action 'metadata)
                         `(metadata (category . mr-x-agent-handoff)
...
staging paths and filtering), presented by `mr-x/agent-handoff--read'
...
latest sessions from the other machines before listing."
          ...
          (let* ((script (expand-file-name
            ...
            (let* ((pl (if (= (length candidates) 1)
                           (car candidates)
                         (mr-x/agent-handoff--read candidates)))
                   (id (plist-get pl :id))
