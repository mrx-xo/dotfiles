...
silent context-only capture with no marker."
  ...
  Cleans up the subscription, timer, and optionally interrupts the shell process."
      ...
      Never less than twice the buffer's line count, the card's worst case."
        (let* ((frame (and (boundp 'posframe--frame) (buffer-local-value 'posframe--frame buf)))
               ...
               (estimate (* 2 (with-current-buffer buf (count-lines (point-min) (point-max))))))
          (if (not (window-live-p window))
              estimate
            (redisplay t)
            ;; nil FROM/TO: the whole of the posframe's own buffer.
            (let ((px (cdr (window-text-pixel-size window nil nil))))
              (if (and (integerp px) (> px 0))
