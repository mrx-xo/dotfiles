;;; -*- lexical-binding: t -*-
      ...
      (use-package agent-shell
        ...
        ;; CMD+Enter to submit prompt from normal OR insert mode (lazy mode)
        (evil-define-key 'normal agent-shell-mode-map (kbd "s-<return>") #'shell-maker-submit)
        (evil-define-key 'insert agent-shell-mode-map (kbd "s-<return>") #'shell-maker-submit)
        ...
        ;; Fenced block -> vterm, no Enter.  The agent says "run this",
        ;; you hit SPC c ! on the block, then RET in the terminal.  The
        ;; text goes in as a bracketed paste so a multi-line block sits
        ;; on the prompt as one unit instead of running line by line.
        (defun mr-x/agent-shell-command-at-point ()
          "Return the shell text under point: region, fenced block body, or line.
A leading `$ ' prompt is dropped from every line.  Signals a
`user-error' when the result is blank."
          (let* ((raw (cond
                       ((use-region-p)
                        (buffer-substring-no-properties
                         (region-beginning) (region-end)))
                       ;; The body tag skips the char after the last
                       ;; line, so an end-of-line point also looks back.
                       ((and (fboundp 'agent-shell-markdown-source-block-at-point)
                             (or (agent-shell-markdown-source-block-at-point)
                                 (and (> (point) (point-min))
                                      (agent-shell-markdown-source-block-at-point
                                       (1- (point)))))))
                 ...
                 (lines (mapcar (lambda (line)
                                  (string-trim
                                   (replace-regexp-in-string
                                    "\\`[ \t]*\\$ " "" line)))
                                (split-string raw "\n")))
                 (text (string-trim (string-join lines "\n"))))
            (when (string-empty-p text)
              (user-error "No command at point"))
            text))

        (defun mr-x/agent-shell-block-to-vterm ()
          "Paste the command at point into a vterm and go there, without running it.
Targets a vterm already showing in this frame, else the most recently
used live vterm, else a fresh multi-vterm popup in this chat's
directory.  Press RET there to run."
          (interactive)
          (let* ((text (mr-x/agent-shell-command-at-point))
                 (dir default-directory)
                 (live (seq-filter (lambda (b)
                                     (with-current-buffer b
                                       (and (derived-mode-p 'vterm-mode)
                                            (get-buffer-process b))))
                                   (buffer-list)))
                 (buf (or (seq-find (lambda (b) (get-buffer-window b)) live)
                          (car live))))
            ...
            (when (fboundp 'evil-insert-state)
              (evil-insert-state))
            (message "Pasted %d line(s), RET to run"
...
\"gemini-api-token\" first and only falls back to Vaultwarden.  The
...
with  secret --cache \"gemini-api-token\"."
          (when-let* ((secret-bin (executable-find "secret"))
                      (command (concat (if-let* ((timeout-bin (executable-find "timeout")))
                                       ...
                                       (shell-quote-argument "gemini-api-token")
                                       " 2>/dev/null"))
        ...
        ;; global `agent-shell-mcp-servers' (ask-user, chrome-devtools
        ;; and friends) into every session; small models drown in a big tool list and
        ...
        ;; MCP Servers injected into every agent-shell session
        ;; Stdio transport requires: name, command, args, env
        ;; env must be a vector [] for proper JSON array serialization
        ;; task-master-ai removed 2026-09-23: unused, and one copy per session
        ;; cost ~60 MB each.
...
coming from the provider untouched."
    ...
    FILE is used for syntax detection. _BUF is ignored (we insert at point)."
      ...
      (with-eval-after-load 'major-pane
        ...
        ;; Labels (and live mode) must survive SPC c y: agent-shell-restart
        ;; kills the buffer and creates a replacement under the same name,
        ;; but major-pane--labels is keyed by buffer OBJECT — the entry
        ;; died with the buffer (observed: label vanishing after one
        ;; resync).  Carry both to the successor; the label-heal timer
        ;; then mirrors the new session id to labels.json within 3s.
        (defun mr-x/agent-label--keep-across-restart (orig &rest args)
          "Carry pane label and live mode across ORIG (`agent-shell-restart')."
...
an aborted invocation reschedules itself like a not-ready one."
        ...
        (defun mr-x/agent-shell--clone-config (source)
          "Copy SOURCE's agent config pinned to its live model + permission mode.
SOURCE is an agent-shell buffer.  This is what `mr-x/agent-shell-clone'
does for the rig; the phone's Clone menu item reaches it through
`mr-x/agent-shell-spawn-for-mobile'."
          (with-current-buffer source
            (unless (derived-mode-p 'agent-shell-mode)
              (error "Not an agent-shell buffer: %s" (buffer-name source)))
        ...
        (defun mr-x/agent-shell-spawn-for-mobile (cwd name task &optional preset clone-of)
...
CLONE-OF, when non-empty, names an existing agent-shell buffer: the new
convo copies its agent, model, permission mode and directory (what
SPC c n does on the rig), CWD may then be empty, and an empty NAME
inherits the source's pane label with a \" 2\" suffix.  A PRESET
still wins over the copied model + mode."
          (let* ((source (and clone-of (not (string-empty-p clone-of))
                              (or (get-buffer clone-of)
                                  (error "No such buffer: %s" clone-of))))
                 (dir (file-name-as-directory
                       (expand-file-name
                        (if (and source (or (null cwd) (string-empty-p cwd)))
            ...
            (let* ((default-directory dir)
                   ...
                   (config (cond
                            (tuple (mr-x/agent-shell--config-with
                                    (nth 2 tuple) (nth 3 tuple) (nth 4 tuple)))
                            (source (mr-x/agent-shell--clone-config source))
                            (t agent-shell-preferred-agent-config)))
                   (label (cond
                           ((and name (not (string-empty-p name))) name)
                           (source
                            (when-let ((src-label (gethash source major-pane--labels)))
                              (concat src-label " 2")))))
        ...
        ;; Idempotent creation: a re-load must not stack duplicate timers,
        ;; and a wedged timer object (observed: next-fire 53h in the past,
        ;; never firing) gets replaced by a fresh one.
        (cancel-function-timers #'mr-x/agent-status-sync)
        (cancel-function-timers #'mr-x/agent-label-heal)
...
silent context-only capture with no marker."
      ...
      (use-package agent-recall
        ...
        ;;
        ;; Only while the buffer is still a registered conversation.  On
        ;; kill, `major-pane--unregister-conversation' runs first on the
        ;; same hook and drops the label from the hash; the capture that
        ;; followed read nil and deleted the stored label of every killed
        ;; chat (observed 2026-09-08: "ANDROID SCEEEN" lost after a kill).
        ;; Unregister also removes the buffer from the conversations list,
        ;; so that membership is the "still alive" signal.
        (add-hook 'agent-recall-capture-functions
                  (defun mr-x/agent-recall-capture-label ()
                    (when (and (boundp 'major-pane--labels)
                               (boundp 'major-pane--state)
                               (memq (current-buffer)
                                     (major-pane-state-conversations
                                      major-pane--state)))
    ...
    - 'hybrid     - Highlighted keys + separator (recommended)")
      ...
      (defvar mr-x/quick-ask--sessions (make-hash-table :test #'equal)
        "Hidden Quick Ask sessions, one per project root.")

      (defun mr-x/quick-ask--project-directory (&optional dir)
        "The root of DIR's project, else DIR; home for remote or missing DIR.
      Quick Ask's agent runs there, so it can read the project it is asked about."
        (let ((dir (and dir (not (file-remote-p dir)) (file-directory-p dir) dir)))
          (file-name-as-directory
           (file-truename
            (or (and dir (ignore-errors
                           (when-let ((project (project-current nil dir)))
                             (project-root project))))
                dir
                "~")))))

      (defun mr-x/quick-ask--ensure-session (&optional dir)
        "Return a healthy agent-shell buffer for DIR's project, creating one if needed.
      It also becomes `mr-x/quick-ask--shell-buffer', the session abort and
      continue-in-chat act on."
        (let* ((root (mr-x/quick-ask--project-directory dir))
               (buf (gethash root mr-x/quick-ask--sessions)))
        ...
        (let* ((root (mr-x/quick-ask--project-directory dir))
          ...
          (when mr-x/quick-ask--shell-buffer
            (with-current-buffer mr-x/quick-ask--shell-buffer
              ...
              (major-pane-exclude-buffer)
              (puthash root (current-buffer) mr-x/quick-ask--sessions)
              (setq-local mr-x/quick-ask--session-root root)
              (add-hook 'kill-buffer-hook #'mr-x/quick-ask--forget-session nil t)))
          mr-x/quick-ask--shell-buffer))

      (defvar-local mr-x/quick-ask--session-root nil
        "Project root of this hidden Quick Ask session.")

      (defun mr-x/quick-ask--forget-session ()
        "Drop the current buffer from the Quick Ask session table."
        (let ((buf (current-buffer)))
          (when (eq (gethash mr-x/quick-ask--session-root mr-x/quick-ask--sessions) buf)
            (remhash mr-x/quick-ask--session-root mr-x/quick-ask--sessions))
          (when (eq mr-x/quick-ask--shell-buffer buf)
            (setq mr-x/quick-ask--shell-buffer nil))))
      ...
      ;; Context picker
      (defvar-local mr-x/quick-ask--source-origin nil
        "Origin captured before the source selection or buffer disappears.")
      (defvar-local mr-x/quick-ask--source-context nil
        "Selected source text without review gutters or padding.")
      (defvar-local mr-x/quick-ask--posframe nil
        "Non-nil when the response is displayed in a child frame.")
      ...
      (with-eval-after-load 'evil
        ...
        (evil-define-key 'normal mr-x/quick-ask-response-map
          ...
          (kbd "c") #'mr-x/quick-ask--surface-session
          (kbd "u") #'mr-x/quick-ask--park
          (kbd "y") #'mr-x/quick-ask--copy
          (kbd "j") #'next-line
          (kbd "k") #'previous-line
          (kbd "G") #'end-of-buffer
  ...
  Cleans up the subscription, timer, and optionally interrupts the shell process."
      ...
      (defun mr-x/quick-ask--dismiss ()
        "Close the response and restore focus to its source window."
        (interactive)
        (let ((buf (get-buffer "*quick-ask*")))
          (when (buffer-live-p buf)
            (let* ((source (buffer-local-value 'mr-x/quick-ask--source-buffer buf))
                   (source-window (and (buffer-live-p source)
                                       (get-buffer-window source t)))
                   (child (buffer-local-value 'mr-x/quick-ask--posframe buf)))
              (with-current-buffer buf
                (when mr-x/quick-ask--thinking-timer
                  (cancel-timer mr-x/quick-ask--thinking-timer))
                (when (and child (fboundp 'posframe-hide))
                  (posframe-hide buf)))
              ;; Do not quit the child frame's sole window.
              (dolist (window (get-buffer-window-list buf nil t))
                (unless (frame-parameter (window-frame window) 'parent-frame)
                  (quit-window nil window)))
              (kill-buffer buf)
              (when (and child (window-live-p source-window))
                (select-window source-window)
                (when (display-graphic-p (window-frame source-window))
                  (select-frame-set-input-focus (window-frame source-window))))))))

      (defun mr-x/quick-ask--park ()
        "Park the question with its captured text and source origin."
        (interactive)
        (require 'syzygy-park)
        (let ((question (or mr-x/quick-ask--question
                            (user-error "No question to park")))
              (context mr-x/quick-ask--source-context)
              (origin mr-x/quick-ask--source-origin)
              (source mr-x/quick-ask--source-buffer))
          (if (buffer-live-p source)
              (with-current-buffer source (syzygy-park question context origin))
            (syzygy-park question context origin))
          (mr-x/quick-ask--dismiss)))

      (defun mr-x/quick-ask--copy ()
        "Copy the answer and dismiss the response."
        (interactive)
        (kill-new (or mr-x/quick-ask--response ""))
        (mr-x/quick-ask--dismiss)
      ...
      Measured after a redisplay: a child frame not yet drawn reports 0 px."
        (let* ((frame (and (boundp 'posframe--frame) (buffer-local-value 'posframe--frame buf)))
               (window (and (frame-live-p frame) (frame-root-window frame))))
          (when (window-live-p window)
            (redisplay t)
            ;; To the end: the last row's padding lives on its newline.
            (let ((px (cdr (window-text-pixel-size window nil nil))))
              (and (integerp px) (> px 0) px)))))

      (defun mr-x/quick-ask--content-lines (buf)
        "Lines of frame height BUF's posframe needs to show all of it.
      Never less than twice the buffer's line count when it cannot be measured."
        (let ((px (mr-x/quick-ask--content-pixels buf))
              (frame (and (boundp 'posframe--frame) (buffer-local-value 'posframe--frame buf))))
          (if (and px (frame-live-p frame))
      ...
      (defun mr-x/quick-ask--show-float (buf source-window)
        ...
        (with-current-buffer buf
          ...
          (let ((source mr-x/quick-ask--source-buffer)
                ...
                ;; posframe hides the cursor unless given one: a bar while
                ;; typing the question, a box over the answer.
                (cursor (if (eq mr-x/quick-ask--phase 'input) 'bar 'box)))
      ...
      (defun mr-x/quick-ask--surface-session ()
        ...
        (let ((shell-buf mr-x/quick-ask--shell-buffer))
          ...
          (if (and shell-buf (buffer-live-p shell-buf))
              (progn
                (setq mr-x/quick-ask--shell-buffer nil)
                ;; It is a normal chat now; the next question starts afresh.
                (maphash (lambda (root buf)
                           (when (eq buf shell-buf) (remhash root mr-x/quick-ask--sessions)))
                         mr-x/quick-ask--sessions)
                (pop-to-buffer shell-buf))
  ...
  Strip everything up to and including the thinking block."
        ...
        (let ((inhibit-read-only t))
          ...
          (if (not response)
            ...
            (let ((clean-response (mr-x/quick-ask--strip-thinking response)))
              ...
              ;; question, answer, and the exits as keycaps along the bottom.
              (setq-local header-line-format nil)
              (require 'review-panel)
              (review-panel-ask-card (plist-get mr-x/quick-ask--source-origin :label)
                                     question clean-response
          ...
          (when (fboundp 'evil-normal-state)
            (evil-normal-state))
          (mr-x/quick-ask--display-response (current-buffer))))
      ...
      (defun mr-x/quick-ask ()
        ...
        (let* ((source-buf (current-buffer))
              ...
              (source-origin (syzygy-park--origin))
              (selection (and region-active (derived-mode-p 'review-pane-mode)
                              (review-session-pane-selection region-beg region-end)))
              (source-context
               (when region-active
                 (if selection (plist-get selection :text)
                   (buffer-substring-no-properties region-beg region-end))))
              ;; A review selection with changes goes to the agent as a small
              ;; diff, so it sees both sides; parking keeps the plain text.
              (source-diff (plist-get selection :diff)))
          (let ((buf (get-buffer-create "*quick-ask*")))
            (with-current-buffer buf
              (let ((inhibit-read-only t))
                ...
                (setq mr-x/quick-ask--source-origin source-origin
                      mr-x/quick-ask--source-context source-context)
                ...
                (when region-active
                  (let* ((content (or source-diff source-context))
                         ...
                         (label (or (plist-get source-origin :label)
                                    (format "%s (lines %d-%d)"
