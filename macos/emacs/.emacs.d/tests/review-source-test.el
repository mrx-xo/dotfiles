...
(ert-deftest review-source-forgejo-labels-each-side-with-branch-and-commit ()
  (review-source-test--with-patch
    (let* ((files (review-source-forgejo-patch-files))
           (src (review-source-forgejo-pr "https://forge.example" "team" "project" 41 files)))
      (cl-letf (((symbol-function 'forgejo-api-get)
                 (lambda (_host path _params cb &rest _args)
                   (if (string-suffix-p "pulls/41" path)
                       (funcall cb '((merge_base . "9c1e2f4aaaa") (base . ((ref . "main")))
                                     (head . ((sha . "41d0aa7bbbb") (ref . "feat/cache"))))
                                nil)
                     (funcall cb `((sha . "2222222abc") (encoding . "base64")
                                   (content . ,(base64-encode-string "new\n")))
                              nil)))))
        (funcall (review-source-text src) (car files) 'new #'ignore)
        (should (equal (review-source-old-label src) "main @ 9c1e2f4"))
        (should (equal (review-source-new-label src) "feat/cache @ 41d0aa7"))))))

(ert-deftest review-source-git-labels-each-side-with-branch-and-commit ()
  (let ((dir (make-temp-file "review-labels" t)))
    (unwind-protect
        (cl-flet ((git (&rest args) (string-trim (apply #'review-source-test--git dir args)))
                  (labels (range)
                    (let ((src (review-source-git-range dir range)))
                      (funcall (review-source-files src))
                      (list (review-source-old-label src) (review-source-new-label src)))))
          (git "init" "-q" "-b" "main")
          ...
          (let ((base (substring (git "rev-parse" "main") 0 7))
                (head (substring (git "rev-parse" "feature") 0 7)))
            (should (equal (labels "main...feature")
                           (list (format "main @ %s" base) (format "feature @ %s" head))))
            ;; HEAD reads as the branch it points at.
            (should (equal (labels "main...HEAD")
                           (list (format "main @ %s" base) (format "feature @ %s" head))))
            (with-temp-file (expand-file-name "a.txt" dir) (insert "three\n"))
            (should (equal (labels nil) (list (format "feature @ %s" head) "working tree")))
            (git "add" "a.txt")
