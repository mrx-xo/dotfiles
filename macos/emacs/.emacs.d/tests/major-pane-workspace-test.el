...
(ert-deftest major-pane-workspace-resume-entry-resumes-when-closed ()
  ...
  (mpw-test--with-convos ()
    (let (resumed)
      (cl-letf (((symbol-function 'syzygy-recall-resume-entry)
                 (lambda (entry _done)
                   (setq resumed (list (plist-get entry :session-id)
                                       (plist-get entry :cwd) (plist-get entry :agent))) nil)))
        (major-pane-workspace--resume-entry
...
(ert-deftest major-pane-workspace-includes-ejected-excludes-workers ()
  (mpw-test--with-convos (("pane" "a" "/tmp/" codex nil nil)
                          ("ejected" "b" "/tmp/" codex nil nil)
                          ("worker" "c" "/tmp/" codex nil nil))
    (let ((ejected (get-buffer "ejected")) (worker (get-buffer "worker")))
      (setf (major-pane-state-conversations major-pane--state) (list (get-buffer "pane")))
      (with-current-buffer ejected (setq-local major-pane--excluded 'ejected))
      (with-current-buffer worker (setq-local major-pane--excluded t))
      (should (equal (mapcar (lambda (e) (plist-get e :session-id))
                            (major-pane-workspace--collect)) '("a" "b")))
      (should (eq (plist-get (cadr (major-pane-workspace--collect)) :membership) 'ejected)))))

(ert-deftest major-pane-workspace-events-save-and-close-without-idle ()
  (mpw-test--with-convos (("event-chat" "a" "/tmp/" codex nil nil))
    (let ((noninteractive nil)
          (major-pane-workspace-mode nil)
          (major-pane-workspace-capture-function nil)
          (major-pane-workspace--timer nil)
          (major-pane-workspace--last-written nil))
      (unwind-protect
          (progn
            (major-pane-workspace-mode 1)
            (major-pane-workspace-request-save)
            (sleep-for 0.05)
            (should (equal (plist-get (car (plist-get (car (major-pane-workspace--read)) :convos)) :session-id) "a"))
            (with-current-buffer (get-buffer "event-chat")
              (major-pane-workspace--watch-buffer)
              (kill-buffer))
            (sleep-for 0.05)
            (should-not (plist-get (car (major-pane-workspace--read)) :convos)))
        (major-pane-workspace-mode -1)))))

(ert-deftest major-pane-workspace-session-ready-and-label-events-persist ()
  (mpw-test--with-convos (("ready-chat" nil "/tmp/" codex nil nil))
    (let ((noninteractive nil) (major-pane-workspace-mode nil)
          (major-pane-workspace-capture-function nil)
          (major-pane-workspace--timer nil) (major-pane-workspace--last-written nil)
          subscriber)
      (cl-letf (((symbol-function 'agent-shell-subscribe-to)
                 (lambda (&rest args) (setq subscriber (plist-get args :on-event)) 42))
                ((symbol-function 'agent-shell-unsubscribe) (lambda (&rest _) nil)))
        (unwind-protect
            (progn
              (with-current-buffer (get-buffer "ready-chat")
                (setq-local major-mode 'agent-shell-mode))
              (major-pane-workspace-mode 1)
              (sleep-for 0.02)
              (should-not major-pane-workspace--boot)
              (with-current-buffer (get-buffer "ready-chat")
                (setf (alist-get :session agent-shell--state) '((:id . "forked-id"))))
              (funcall subscriber '((:event . init-session)))
              (sleep-for 0.02)
              (should (equal (plist-get (car (plist-get (car (major-pane-workspace--read)) :convos)) :session-id) "forked-id"))
              (major-pane-set-buffer-label (get-buffer "ready-chat") "Phone label")
              (sleep-for 0.02)
              (should (equal (plist-get (car (plist-get (car (major-pane-workspace--read)) :convos)) :label) "Phone label")))
          (major-pane-workspace-mode -1))))))

(ert-deftest major-pane-workspace-capture-failure-keeps-history ()
  (mpw-test--with-convos (("fail-chat" "sid" "/tmp/" codex nil nil))
    (major-pane-workspace-save)
    (let ((before (major-pane-workspace--read))
          (major-pane-workspace-capture-function (lambda (_) (error "disk failure"))))
      (major-pane-set-buffer-label (get-buffer "fail-chat") "not committed")
      (major-pane-workspace--flush)
      (should (equal before (major-pane-workspace--read)))
      (should (string-match-p "disk failure" major-pane-workspace-last-error)))))
